...
This approach is error-prone if you have heavy initialization logic OR wanted to start using the API straight after applying the middleware - in this case, requests would not be handled until the `1000` milisecond `setTimeout` passes and applies the request middleware.
